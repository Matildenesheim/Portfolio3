...
m4 = glmer(Diagnosis ~ rangeScale  +(1|Study) , df, family = "binomial",  control = glmerControl(calc.derivs = FALSE))
...
```{r}
#Creating function for automatic crossvalidation
...
##Creating training and test sets. Using CV on the training set and testing the models on the test set


# Split data in 20/80 (percentage)
parts = partition(df, p = 0.2, id_col = "Subject", cat_col = 'Diagnosis')
...
IQRm = cv(train, nfolds, mIQR, "Diagnosis")
...
MADm = cv(train, nfolds, mMAD, "Diagnosis")
mean(MADm$AUC)

##Using Ludvigs lovely cv function for cross validating multiple functions at once
mixed_models <- c("Diagnosis~meanS+(1|Study)",
                  "Diagnosis~medianS+(1|Study)",
                  "Diagnosis~sdS+(1|Study)",
                  "Diagnosis~rangeScale+(1|Study)",
                  "Diagnosis~iqrS+(1|Study)",
                  "Diagnosis~madS+(1|Study)",
                  "Diagnosis~coefvarS+(1|Study)",
                  ...
                  "Diagnosis~nsyllS+(1|Study)"
                  )
CV <- cross_validate(train, mixed_models, 
                     folds_col = '.folds', 
                     family='binomial', 
                     REML = FALSE)
...
CV[max,]$Fixed
View(CV)
#coefvarS seems to be the best predictor (except not really, it changes depending on how the data gets randomly grouped)
```
...
CV2 <- cross_validate(train, models2, 
                     folds_col = '.folds', 
                     family='binomial', 
                     REML = FALSE)
...
#creating summary statistics and confusin matrix for the 3 best performing models: 
#Diagnosis ~ meanS + Ls + DETs + coefvarS + (1|Study)
m1= glmer(Diagnosis ~ meanS + Ls + DETs + coefvarS + (1|Study), train, family = 'binomial')
summary(m1)
