...
setwd("~/Desktop/OneDrive/Cognitive Science/3 semester/Experimental Methods 3/Portfolio_3")


#loading files
arti = read.csv("Articulation.txt", sep=",")
demo = read.delim("DemoData.txt")
pitch1 = read.delim("~/Desktop/OneDrive/Cognitive Science/3 semester/Experimental Methods 3/Portfolio_3/Pitch/Study1D0S101T2_f0.txt")
...
files = list.files(path = "~/Desktop/OneDrive/Cognitive Science/3 semester/Experimental Methods 3/Portfolio_3/Pitch/")
#saving the folder name
folder = "~/Desktop/OneDrive/Cognitive Science/3 semester/Experimental Methods 3/Portfolio_3/Pitch/"
...
write.csv(all_df, "combinedSchizoData.csv") # write only once. 
...
#woops, time isn't time but actually which video they saw
...
mDET = lmer(DET ~ Diagnosis + time + (1+time|Subject), df)
mRR = lmer(RR ~ Diagnosis + time+ (1+time|Subject), df)
mNRLINE = lmer(NRLINE~ Diagnosis + time + (1+time|Subject), df)
mmaxL = lmer(maxL ~ Diagnosis + time+ (1+time|Subject), df)
mL = lmer(L ~ Diagnosis + time + (1+time|Subject), df) #failed to converge
mENTR = lmer(ENTR ~ Diagnosis + time + (1+time|Subject), df)
mrENTR = lmer(rENTR ~ Diagnosis + time + (1+time|Subject), df)
mLAM = lmer(LAM ~ Diagnosis + time + (1+time|Subject), df)
mTT = lmer(TT ~ Diagnosis + time + (1+time|Subject), df)
mMean = lmer(mean ~ Diagnosis + time + (1+time|Subject), df)
...
mDET = lmer(DET ~ Diagnosis + Study + time + (1+time|Subject), df)
mRR = lmer(RR ~ Diagnosis + Study + time+ (1+time|Subject), df)
mNRLINE = lmer(NRLINE~ Diagnosis + Study + time + (1+time|Subject), df)
mmaxL = lmer(maxL ~ Diagnosis + Study + time+ (1+time|Subject), df)
mL = lmer(L ~ Diagnosis + Study + time + (1+time|Subject), df)
mENTR = lmer(ENTR ~ Diagnosis + Study + time + (1+time|Subject), df)
mrENTR = lmer(rENTR ~ Diagnosis + Study + time + (1+time|Subject), df) 
mLAM = lmer(LAM ~ Diagnosis + Study + time + (1+time|Subject), df)
mTT = lmer(TT ~ Diagnosis + Study + time + (1+time|Subject), df)
mMean = lmer(mean ~ Diagnosis + Study + time + (1+time|Subject), df)
